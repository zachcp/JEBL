...
public class TreeSimulator {
     ...
     * @param taxonCount
     */
    public TreeSimulator(IntervalGenerator intervalGenerator, int taxonCount) {
        this(intervalGenerator, new int[] { taxonCount}, new double[] { 0.0 } );
    }

    public TreeSimulator(IntervalGenerator intervalGenerator, int[] samplingCounts, double[] samplingTimes) {
        ...
        for (int i = 0; i < samplingCounts.length; i++) {
            for (int j = 0; j < samplingCounts[i]; j++) {
                Taxon taxon = Taxon.getTaxon("tip" + Integer.toString(k + 1));
    ...
    public static void main(String[] args) {
        ...
        TreeSimulator sim = new TreeSimulator(intervals, samplingCounts, samplingTimes);
        ...
        try {
            ...
            System.err.println("Simulating " + REPLICATE_COUNT + " trees of " + samplingCounts.length + " tips:");
            System.err.print("[");
