...
public class TreePane extends JComponent implements PainterListener, Printable {
    ...
    public void collapseSelectedNodes() {
        if (selectedNodes.size() == 1) {
            ...
            if (selectedNode.getAttribute(COLLAPSE_ATTRIBUTE_NAME) != null) {
                selectedNode.removeAttribute(COLLAPSE_ATTRIBUTE_NAME);
            } else {
                int tipCount = RootedTreeUtils.getTipCount(tree, selectedNode);
                double height = RootedTreeUtils.getMinTipHeight(tree, selectedNode);
                Object[] values = new Object[] { tipCount, height };
                selectedNode.setAttribute(COLLAPSE_ATTRIBUTE_NAME, values);
            }
    ...
    public void setNodeBarPainter(NodeBarPainter nodeBarPainter) {
        ...
        calibrated = false;
        this.nodeBarPainter.setupAttributes(tree);
        repaint();
    }
        ...
        for (Node node : treeLayout.getBranchPathMap().keySet() ) {
            ...
            if (colouring != null) {
                ...
                for (int i = 0; i < colouring.length - 1; i+=2) {
                    ...
                    int colour = ((Number)colouring[i]).intValue();
                    g2.setPaint(colour == 0 ? Color.BLUE : Color.RED);
                ...
                int colour = ((Number)colouring[colouring.length - 1]).intValue();
                g2.setPaint(colour == 0 ? Color.BLUE : Color.RED);
